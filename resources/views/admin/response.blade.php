...
<article class="form-card response-detail">
    ...
    <div class="d-flex gap-2">
        <form method="post" action="{{ route('admin.responses.unread',$response) }}">@csrf @method('PATCH')<button class="btn btn-outline-secondary"><i class="bi bi-envelope"></i> Mark unread</button></form>
        <form method="post" action="{{ route('admin.responses.destroy',$response) }}" onsubmit="return confirm('Delete this response?')">@csrf @method('DELETE')<button class="btn btn-outline-danger"><i class="bi bi-trash"></i> Delete</button></form>
    </div>
</article>
@endsection
