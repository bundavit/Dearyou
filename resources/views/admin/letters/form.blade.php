...
<div class="col-md-8"><label class="form-label">Optional image or animation</label><input type="file" name="image" class="form-control @error('image') is-invalid @enderror" accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,.webm,image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" data-image-upload><div class="form-text">JPG, PNG, WebP, GIF, MP4, or WebM up to 10 MB. Telegram GIFs usually save as MP4 or WebM and loop automatically.</div>@error('image')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
<div class="col-md-5"><label class="form-label">Memory pictures or animations</label><input class="form-control @error('memory_images') is-invalid @enderror" type="file" name="memory_images[]" multiple accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,.webm,image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" data-image-upload><div class="form-text">Choose up to 10 JPG, PNG, WebP, GIF, MP4, or WebM files, 10 MB each.</div>@error('memory_images')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror @error('memory_images.*')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
@if($memory->images->isNotEmpty())<div class="col-12"><div class="memory-image-editor" data-sortable-images data-reorder-url="{{ route('admin.memory-images.reorder',$memory) }}">@foreach($memory->images as $image)<label draggable="true" data-image-id="{{ $image->id }}">@if(\App\Models\Letter::isVideoMediaPath($image->image_path))<video class="memory-thumb" src="{{ Storage::url($image->image_path) }}" autoplay muted loop playsinline aria-label="Memory video"></video>@else<img class="memory-thumb" src="{{ Storage::url($image->image_path) }}" alt="">@endif<span><i class="bi bi-grip-horizontal" aria-hidden="true"></i><input class="form-check-input" type="checkbox" name="remove_memory_images[]" value="{{ $image->id }}"> Remove</span></label>@endforeach</div></div>@endif
...
<div class="col-md-4"><input class="form-control" type="date" name="memory_date" value="{{ $memory->memory_date?->format('Y-m-d') }}"><input class="form-control mt-2 @error('memory_images') is-invalid @enderror" type="file" name="memory_images[]" multiple accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,.webm,image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" data-image-upload><div class="form-text">Add more pictures, GIFs, MP4s, or WebMs</div>@error('memory_images')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror @error('memory_images.*')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
