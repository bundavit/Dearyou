...
<div class="col-md-8"><label class="form-label">Optional image, GIF, or MP4</label><input type="file" name="image" class="form-control @error('image') is-invalid @enderror" accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,image/jpeg,image/png,image/webp,image/gif,video/mp4" data-image-upload><div class="form-text">JPG, PNG, WebP, animated GIF, or MP4 up to 10 MB. GIFs and MP4s loop automatically in the letter.</div>@error('image')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
@if($letter->image_path)<div class="col-12"><div class="image-preview-row" data-removable-image>@if(str_ends_with(strtolower($letter->image_path), '.mp4'))<video src="{{ Storage::url($letter->image_path) }}" autoplay muted loop playsinline aria-label="{{ $letter->image_alt ?: 'Current letter video' }}"></video>@else<img src="{{ Storage::url($letter->image_path) }}" alt="{{ $letter->image_alt ?: 'Current letter image' }}">@endif<div class="image-preview-actions"><input type="checkbox" name="remove_image" value="1" hidden data-remove-image-input><button class="btn btn-sm btn-outline-danger" type="button" data-remove-image-button><i class="bi bi-trash"></i> <span>Delete picture</span></button><small class="text-secondary" data-remove-image-note>The picture will be deleted when you save the letter.</small></div></div></div>@endif
<div class="col-md-8"><label class="form-label">Optional background music</label><input type="file" name="audio" class="form-control @error('audio') is-invalid @enderror" accept=".mp3,.wav,.ogg,.m4a,.aac,audio/mpeg,audio/wav,audio/ogg,audio/mp4,audio/aac" data-audio-upload><div class="form-text">MP3, WAV, OGG, M4A, or AAC up to 12 MB. Music loops automatically when the visitor's browser allows it.</div>@error('audio')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
<div class="col-md-4"><label class="form-label">Your profile image</label><input type="file" name="sender_profile" class="form-control @error('sender_profile') is-invalid @enderror" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" data-image-upload data-chapter-sender-image><div class="form-text">Up to 10 MB.</div>@error('sender_profile')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
<div class="col-md-4"><label class="form-label">Recipient profile image</label><input type="file" name="recipient_profile" class="form-control @error('recipient_profile') is-invalid @enderror" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" data-image-upload data-chapter-recipient-image><div class="form-text">Up to 10 MB.</div>@error('recipient_profile')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
<div class="col-md-5"><label class="form-label">Memory pictures, GIFs, or MP4s</label><input class="form-control @error('memory_images') is-invalid @enderror" type="file" name="memory_images[]" multiple accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,image/jpeg,image/png,image/webp,image/gif,video/mp4" data-image-upload><div class="form-text">Choose up to 10 pictures, animated GIFs, or MP4s, 10 MB each.</div>@error('memory_images')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror @error('memory_images.*')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
...
@if($memory->images->isNotEmpty())<div class="col-12"><div class="memory-image-editor" data-sortable-images data-reorder-url="{{ route('admin.memory-images.reorder',$memory) }}">@foreach($memory->images as $image)<label draggable="true" data-image-id="{{ $image->id }}">@if(str_ends_with(strtolower($image->image_path), '.mp4'))<video class="memory-thumb" src="{{ Storage::url($image->image_path) }}" autoplay muted loop playsinline aria-label="Memory video"></video>@else<img class="memory-thumb" src="{{ Storage::url($image->image_path) }}" alt="">@endif<span><i class="bi bi-grip-horizontal" aria-hidden="true"></i><input class="form-check-input" type="checkbox" name="remove_memory_images[]" value="{{ $image->id }}"> Remove</span></label>@endforeach</div></div>@endif
...
<div class="col-md-4"><input class="form-control" type="date" name="memory_date" value="{{ $memory->memory_date?->format('Y-m-d') }}"><input class="form-control mt-2 @error('memory_images') is-invalid @enderror" type="file" name="memory_images[]" multiple accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,image/jpeg,image/png,image/webp,image/gif,video/mp4" data-image-upload><div class="form-text">Add more pictures, GIFs, or MP4s</div>@error('memory_images')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror @error('memory_images.*')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
